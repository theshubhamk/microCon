...
??_main:	; 1 bytes @ 0x0
	ds   2
	global	main@mask
main@mask:	; 2 bytes @ 0x2
	ds   2
	global	main@mask1
main@mask1:	; 2 bytes @ 0x4
	ds   2
	global	main@mask2
main@mask2:	; 2 bytes @ 0x6
	ds   2
	global	main@mask3
main@mask3:	; 2 bytes @ 0x8
	ds   2
	global	main@j
main@j:	; 2 bytes @ 0xA
	ds   2
...
;;  j               2   10[COMRAM] int 
;;  mask3           2    8[COMRAM] unsigned int 
;;  mask2           2    6[COMRAM] unsigned int 
;;  mask1           2    4[COMRAM] unsigned int 
;;  mask            2    2[COMRAM] unsigned int 
...
l633:
;main.c: 20: {
;main.c: 22: PORTB = 0xFF;
	setf	((c:3969)),c	;volatile
	line	23
	
l635:
;main.c: 23: unsigned int mask = 0, mask1 = 0x80, mask2 = 0x01, mask3 = 0;
	movlw	high(0)
	movwf	((c:main@mask+1)),c
	movlw	low(0)
	movwf	((c:main@mask)),c
	
l637:
	movlw	high(080h)
	movwf	((c:main@mask1+1)),c
	movlw	low(080h)
	movwf	((c:main@mask1)),c
	
l639:
	movlw	high(01h)
	movwf	((c:main@mask2+1)),c
	movlw	low(01h)
	movwf	((c:main@mask2)),c
	
l641:
	movlw	high(0)
	movwf	((c:main@mask3+1)),c
	movlw	low(0)
	movwf	((c:main@mask3)),c
	line	25
	
l643:
;main.c: 24: int j;
;main.c: 25: for(j = 0; j < 16;j++)
	movlw	high(0)
	movwf	((c:main@j+1)),c
	movlw	low(0)
	movwf	((c:main@j)),c
...
l651:
;main.c: 28: {
;main.c: 29: PORTB = mask;
	movff	(c:main@mask),(c:3969)	;volatile
	line	30
	
l653:
;main.c: 30: mask = mask | mask1;
	movf	((c:main@mask1)),c,w
	iorwf	((c:main@mask)),c,w
	movwf	((c:main@mask)),c
	movf	((c:main@mask1+1)),c,w
	iorwf	((c:main@mask+1)),c,w
	movwf	1+((c:main@mask)),c
	line	31
;main.c: 31: mask1 = mask1 >> 1;
	bcf	status,0
	rrcf	((c:main@mask1+1)),c,w
	movwf	(??_main+0+0)&0ffh,c
	rrcf	((c:main@mask1)),c,w
	movwf	((c:main@mask1)),c
	movff	??_main+0+0,((c:main@mask1+1))
	line	32
	
l655:
;main.c: 32: for(i = 50000;i--;);
	movlw	high(0C350h)
	movwf	((c:_i+1)),c
	movlw	low(0C350h)
	movwf	((c:_i)),c
...
l659:
;main.c: 35: else if(j == 7)
		movlw	7
	xorwf	((c:main@j)),c,w
iorwf	((c:main@j+1)),c,w
	btfss	status,2
	goto	u41
	goto	u40
...
;main.c: 39: else
;main.c: 40: {
;main.c: 41: PORTB = mask3;
	movff	(c:main@mask3),(c:3969)	;volatile
	line	42
	
l663:
;main.c: 42: mask3 = mask3 | mask2;
	movf	((c:main@mask2)),c,w
	iorwf	((c:main@mask3)),c,w
	movwf	((c:main@mask3)),c
	movf	((c:main@mask2+1)),c,w
	iorwf	((c:main@mask3+1)),c,w
	movwf	1+((c:main@mask3)),c
	line	43
;main.c: 43: mask2 = mask2 << 1;
	bcf	status,0
	rlcf	((c:main@mask2)),c,w
	movwf	((c:main@mask2)),c
	rlcf	((c:main@mask2+1)),c,w
	movwf	1+((c:main@mask2)),c
	line	44
	
l665:
;main.c: 44: for(i = 50000;i--;);
	movlw	high(0C350h)
	movwf	((c:_i+1)),c
	movlw	low(0C350h)
	movwf	((c:_i)),c
